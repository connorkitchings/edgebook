...
- **Description:** An enthusiast who wants to test college football betting strategies, record bets with confidence ratings and reasoning, and mathematically analyze their profit-and-loss performance without putting actual capital at risk.
- **Pain Points:** Hard to track bankroll history across spreadsheet tools, lack of post-settlement analytics on confidence calibration, and absence of an audit-proof transaction ledger.
- **Goals:** Accurately log simulated bets, evaluate ROI, filter performance by market type, and review cognitive biases.
...
**Story 2:** As Dana, I want to enter a game, record a bet (stake, odds, confidence, reasoning), and later update the game's final score to settle the bet. (Priority: Must-have, Phase 1)
...
**Story 4:** As Dana, I want to see basic performance stats like ROI, net units won/lost, and performance split by confidence rating. (Priority: Should-have, Phase 3)
...
| CFB schema changes complicate future paper-investing adaptations | Medium | Medium | Maintain a strict codebase separation between the `ledger` module and the `sportsbook` module |
...
|------|----------|-------------------|-------------------|
| 2026-07-10 | Adopt Modular Monolith architecture | Avoid microservices complexity while maintaining code separation | Create `ledger` and `sportsbook` directories inside `src/edgebook` in future phases |
